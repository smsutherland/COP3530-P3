...
void draw() {
    ...
    drawAlert();
    if(arr != null){
        drawHistogram(arr, 50, 450);
        text("Time Taken:", 400, 200);
        text(timeMessage, 400, 225);
    }
...
void drawHistogram(JSONArray histogramData, int x, int y, color axisColor, color bucketColor){
    ...
    text(maxHeight, x-50, y-275);
    text(maxHeight/2, x-50, y-257/2);
    text("# of seconds", x-45, y + 40);

    rotate(-PI/2);
    text("# of solves", - y + 5, 25);
    rotate(PI/2);
}
...
void execute(){
    ...
    try{
        ...
        Process pr = Runtime.getRuntime().exec("..\\..\\backend\\main.exe" + paramsStr);
        pr.waitFor();
        Instant end = Instant.now();
        println("done!");
        arr = loadJSONArray("HistogramOutput.json");

        timeMessage = Duration.between(start, end).toString().substring(2);
    }
